...
def plot_1D_1D(model, cls_data, plot_in_axis, plot_out_axis):
    ...
    if model.interp_method in ("linear", "nonlinear", "gaussian", "gaussian2"):
...
def plot_modes(model, cls_data, plot_in_axis, plot_out_axis):
    ...
    if isinstance(params, list):
        M = max(param.shape[0] for param in params) # max rank
        I = len(params) # dim
        J = params[0].shape[2] # nnode
    else:
    ...
    for idx, ax in enumerate(axes.flat):
        ...
        if isinstance(params, list):
            if m < params[i].shape[0]:
                u_nds = params[i][m,plot_out_axis[0],:] # (J,)
