...
class NonlinearInterpolator(LinearInterpolator):
  def __init__(self, grid, config):
    ...
    self.radial_basis = config['MODEL_PARAM'].get('radial_basis', 'cubicSpline')
    ...
    self.a_dil = config['MODEL_PARAM'].get('length_scale')
    if self.a_dil is None:
      self.alpha_dil = config['MODEL_PARAM']['alpha_dil']
      self.a_dil = self.alpha_dil * self.d_c
    else:
      self.alpha_dil = self.a_dil / self.d_c
