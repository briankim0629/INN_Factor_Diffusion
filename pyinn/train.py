...
INN_INTERP_METHODS = ("linear", "nonlinear", "gaussian", "gaussian2")
...
class Regression_INN:
    def __init__(self, cls_data, config):
        ...
        rank_config = config['MODEL_PARAM'].get('tucker_rank', config['MODEL_PARAM'].get('nmode'))
        if isinstance(rank_config, list):
            if len(rank_config) != self.cls_data.dim:
                raise ValueError(f"tucker_rank length {len(rank_config)} must match input dim {self.cls_data.dim}")
            self.tucker_ranks = [int(rank) for rank in rank_config]
            self.nmode = max(self.tucker_ranks)
        else:
            self.nmode = int(rank_config)
            self.tucker_ranks = [self.nmode] * self.cls_data.dim
        ...
        if config['TD_type'] == 'Tucker':
            model_grid = [self.x_dms_nds[idm, :] for idm in range(self.cls_data.dim)]
        else:
            model_grid = self.x_dms_nds[0, :]

        if self.interp_method == "linear":
            model = INN_linear(model_grid, config)
            self.forward = model.forward
            self.v_forward = model.v_forward
            self.vv_forward = model.vv_forward
        elif self.interp_method == "nonlinear":
            model = INN_nonlinear(model_grid, config)
            self.forward = model.forward
            self.v_forward = model.v_forward
            self.vv_forward = model.vv_forward
        elif self.interp_method in ("gaussian", "gaussian2"):
            model = INN_gaussian(model_grid, config)
            self.forward = model.forward
            self.v_forward = model.v_forward
            self.vv_forward = model.vv_forward
        ...
        if config['TD_type'] == 'Tucker':
            self.params = [
                jax.random.uniform(
                    jax.random.PRNGKey(self.key + 100 + idm),
                    (rank_idm, self.cls_data.var, self.nnode),
                    dtype=jnp.double,
                ) * (1.0 / rank_idm ** (1.0 / self.cls_data.dim))
                for idm, rank_idm in enumerate(self.tucker_ranks)
            ]
            numParam = sum(rank_idm*self.cls_data.var*self.nnode for rank_idm in self.tucker_ranks)

            shape = tuple(self.tucker_ranks)
            indices = jnp.arange(min(self.tucker_ranks))

            core_noise_scale = float(config['MODEL_PARAM'].get('core_init_noise', 1e-2))
            core = core_noise_scale * jax.random.normal(
                jax.random.PRNGKey(self.key + 1),
                shape,
                dtype=jnp.double,
            )
            core = core.at[tuple([indices] * self.cls_data.dim)].add(1.0)
            ...
            if isinstance(rank_config, list):
                raise ValueError("Per-dimension tucker_rank is only supported when TD_type is Tucker")
        ...
        if self.interp_method in INN_INTERP_METHODS:
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            print(f"\tTraining RMSE: {np.sqrt(batch_loss_train):.4e}")
            ...
            if (epoch+1)%self.validation_period == 0:
                ...
                for batch in self.val_dataloader:
                    ...
                    loss_val, u_pred_val = self.get_loss(params, x_val, u_val)
                ...
                print(f"\tValidation RMSE: {np.sqrt(batch_loss_val):.4e}")
        ...
        for batch in self.test_dataloader:
            ...
            loss_test, u_pred_test = self.get_loss(params, x_test, u_test)
        ...
        print("Test")
        print(f"\tTest RMSE: {np.sqrt(batch_loss_test):.4e}")
