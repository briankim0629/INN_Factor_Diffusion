...
try:
    from .model import INN_linear, INN_nonlinear, INN_gaussian
...
def save_model_data(config, data, params, data_name, interp_method):
    ...
    config_to_save['data_name'] = data_name
    config_to_save['interp_method'] = interp_method
...
def create_model_from_saved_data(model_data, run_type):
    ...
    if interp_method == "linear":
        ...
        model = INN_gaussian(grid_dms, config)

    elif interp_method == "MLP":
        raise NotImplementedError("MLP model loading is not implemented in this pyinn_tucker model_utils.")
